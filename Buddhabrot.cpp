#define _CRT_SECURE_NO_DEPRECATE // to avoid https://stackoverflow.com/questions/14386/fopen-deprecated-warning
#include <iostream>
#include <tuple>
#include <algorithm> 
#include <fstream>
...
std::tuple<int, int, int> find_rgb(double a, const std::string& palette) {
	...
	if (palette == "Sunset") {
		...
		r = (int)(128 + 127 * std::cos((a - 0.166) * 4.6));
		g = (int)(128 + 127 * std::cos((a + 0.166) * 4.6));
		b = (int)(128 + 127 * std::cos((a + 0.500) * 4.6));
	}
...
public:
	...
	Buddhabrot(const std::string& filename) {
		...
		std::vector<char> buffer(1, 0); //reads only the first 1024 bytes
		inFile >> pointsDoneTot >> min >> max >> xDet >> yDet >> xMin >> xMax >> yMin >> yMax;
		count.reserve(xDet * yDet);
		int val;
		while (!inFile.eof()) {
	...
	void CalcDouble(int pointsToDo, bool countDist) {
		...
		while (pointsDone < pointsToDo) {
			...
			if (iter >= min && iter <= max) {// && int(iter) % 2 == 0) {
				re = re0;
				im = im0;
				...
				pointsDoneTot++;
				pointsDone++;
				if (std::fmod(pointsDone, dispInterval) < 1) {
	...
	int FindPercentileVal(float percentile) {
		...
		if (true) { // Print stuff to help me think about how to crop outliers
			std::ofstream myfile;
			myfile.open("test.csv");

			float pointsCounted = 0;
			float percentsCounted = 0;
			for (int i = 0; i <= maxVal; i++) {
				pointsCounted += histogram[i];
				while (percentsCounted / 1000. <= pointsCounted / pointsTot) {
					myfile << i << "\n";
					percentsCounted++;
				}
			}
	...
	void PrintBMP(const std::string& filename, const std::string& palette, const std::string& trimMethod, double trimVal, float powVal, bool mirror, float gradVal) {
		...
		std::ostringstream oss;
		oss << filename << " [" << xDet << "x" << yDet << "] [" << min << "-" << max << "] " << pointsDoneTot << " points [" << trimMethod << " " << trimVal << " pow " << powVal << "].bmp";
		std::string fullFilename = oss.str();
		...
		double a;
		if (trimMethod == "Trim") trimVal = FindPercentileVal(trimVal);
		...
		for (x = xDet - 1; x >= 0; x--) {
			for (y = yDet - 1; y >= 0; y--) {					// BMP image format is written from bottom to top
				...
				// Mirror horizontally 
				if (mirror) {
					a += count[x * yDet + (yDet - y - 1)];
				}

				// Apply correct Transformation to correct pixel magnitude distribution
				if (trimMethod == "Trim") a = std::min(a, trimVal) / (trimVal * 1.);
				if (trimMethod == "Logistic") a = 2. / (1. + exp(-a / trimVal)) - 1;

				// Apply power to correct pixel magnitude distribution
				a = pow(a, powVal);

				// Add gradient
				if (gradVal > 0) {
					float xT = (float)(xDet) * 0.5;
					float yT = (float)(yDet) * 0.5;
					float xR = ((float)(x)-xT) / xT;
					float yR = ((float)(y)-yT) / yT;
					float rad = xR * xR + yR * yR;
					if (rad > 1) { rad = 0; }
					a += rad * gradVal;
				}

				// Convert scalar into RGB and save to image
				std::tuple<int, int, int> rgb = find_rgb(a, palette);
...
int main() {
	...
	Buddhabrot BB(minVal, maxVal, canvasSize, canvasSize, -1.999, 1.999, -1.999, 1.999);//-1.6, 1.2, -1.4, 1.4);

	// Calculate Buddhabrot
	BB.CalcDouble(numVals, false);
	BB.SaveMatrix("Buddhabrot");

	// Or load existing text file
	Buddhabrot BB("Buddhabrot [2500x2500] [12-36] 1e+8 points.txt");

	// Print buddhabrot
	bool mirror = true;
	float logisticVal = numVals * ((maxVal - minVal) / (log(maxVal) - log(minVal))) * (1 + mirror) / (canvasSize * canvasSize) * 3; 
	float powVal = 1.3;
	float gradVal = 0.1;
	BB.PrintBMP("printboibrot", "Gold", "Logistic", logisticVal, 1.2, mirror, gradVal);
